...
import { Check, X, Eye, Coins, Plus, RefreshCw, Filter } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const { data: approvedData, isLoading: loadingApproved, refetch: refetchApproved } = useQuery({
    queryKey: ['admin-signals-approved', filters],
    queryFn: () => adminService.getApprovedForMinting({ symbol: filters.symbol, aiModel: filters.aiModel, sortBy: 'newest', limit: filters.limit ?? 50 }),
  });
  ...
  const mintMutation = useMutation({
    mutationFn: (signalId: string) => adminService.mintSignalAsParent(signalId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-signals-approved'] });
      setSelectedSignal(null);
    },
  ...
  const handleMint = (signalId: string) => {
    mintMutation.mutate(signalId);
  };
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Button variant="secondary" onClick={() => { refetchPending(); refetchApproved(); }} disabled={isLoading}>
      ...
      {analytics && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg border border-neutral-200">
            <h3 className="text-sm font-medium text-neutral-500">Total Improvements</h3>
            <p className="text-2xl font-bold text-neutral-900">{analytics.totalImprovements}</p>
          </div>
          <div className="bg-white p-4 rounded-lg border border-neutral-200">
          ...
          <div className="bg-white p-4 rounded-lg border border-neutral-200">
            <h3 className="text-sm font-medium text-neutral-500">Active Users</h3>
            <p className="text-2xl font-bold text-neutral-900">{analytics.activeUsers}</p>
          </div>
        </div>
      )}

      {showFilters && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-white p-4 rounded-lg border border-neutral-200 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFilters((prev) => ({ ...prev, minConfidence: e.target.value ? Number(e.target.value) : undefined }))}
              ...
              <div key={signal.id} className="bg-white p-4 rounded-lg border border-neutral-200">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-neutral-600">{signal.side.toUpperCase()} • {signal.confidence}% confidence</p>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${signal.side === 'long' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {signal.side.toUpperCase()}
                  </span>
                </div>

                <div className="grid grid-cols-3 gap-3 text-sm mb-3">
                  <div><p className="text-neutral-500">Entry</p><p className="font-medium">${signal.entryPrice}</p></div>
                  <div><p className="text-neutral-500">Stop Loss</p><p className="font-medium">${signal.stopLoss || 'N/A'}</p></div>
                  <div><p className="text-neutral-500">Take Profit</p><p className="font-medium">${signal.takeProfit || 'N/A'}</p></div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button size="sm" variant="primary" onClick={() => handleApprove(signal.id)} disabled={approveMutation.isPending} className="flex items-center bg-green-600 hover:bg-green-700">
                    <Check className="w-3 h-3 mr-1" />
                    Approve
                  </Button>
                  <Button size="sm" variant="secondary" onClick={() => { setSelectedSignal(signal); setAdminNotes(''); }} disabled={rejectMutation.isPending} className="flex items-center text-red-600 border-red-200 hover:bg-red-50">
              ...
              <div key={signal.id} className="p-4 rounded-lg border border-green-200 bg-green-50">
                ...
                {signal.adminNotes && (
                  <div className="bg-white p-2 rounded border mb-3">
                    <p className="text-sm text-neutral-600"><strong>Admin Notes:</strong> {signal.adminNotes}</p>
                  </div>
                )}

                <Button size="sm" variant="primary" onClick={() => handleMint(signal.id)} disabled={mintMutation.isPending} className="flex items-center">
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-sm text-neutral-500">Direction</p><p className="font-medium">{selectedSignal.side.toUpperCase()}</p></div>
                <div><p className="text-sm text-neutral-500">Confidence</p><p className="font-medium">{selectedSignal.confidence}%</p></div>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div><p className="text-sm text-neutral-500">Entry Price</p><p className="font-medium">${selectedSignal.entryPrice}</p></div>
                <div><p className="text-sm text-neutral-500">Stop Loss</p><p className="font-medium">${selectedSignal.stopLoss || 'N/A'}</p></div>
                <div><p className="text-sm text-neutral-500">Take Profit</p><p className="font-medium">${selectedSignal.takeProfit || 'N/A'}</p></div>
              </div>
              ...
              <div>
                ...
                <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} className="w-full px-3 py-2 border border-neutral-200 rounded-md" rows={3} placeholder="Add notes for approval/rejection..." />
              </div>
            </div>

            <div className="p-6 border-t border-neutral-200 flex items-center space-x-3">
              <Button variant="secondary" onClick={() => { setSelectedSignal(null); setAdminNotes(''); }}>Close</Button>
              <Button variant="primary" onClick={() => handleApprove(selectedSignal.id, adminNotes)} disabled={approveMutation.isPending} className="bg-green-600 hover:bg-green-700">
                <Check className="w-4 h-4 mr-2" />
                Approve
              </Button>
              <Button variant="secondary" onClick={() => handleReject(selectedSignal.id, adminNotes)} disabled={rejectMutation.isPending || !adminNotes.trim()} className="text-red-600 border-red-200 hover:bg-red-50">
